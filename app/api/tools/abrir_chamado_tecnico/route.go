...
func POST(c *trilha.Ctx) error {
	...
	case len(nome) < 3 || len(nome) > 120:
		return c.JSON(http.StatusOK, falha("nome_invalido",
			"Preciso do nome completo do cliente para abrir o chamado."))
	...
	att := store.Criar(atendimento.TipoChamadoTecnico, cpf, resumoDo(problema, descricao),
	...
	return c.JSON(http.StatusOK, resposta{
		...
		Mensagem: "Chamado " + att.Protocolo + " aberto com prioridade " + prioridade.Rotulo() +
			". " + prazoDe(prioridade) + ", e pode acompanhar tudo pelo protocolo.",
	})
}
